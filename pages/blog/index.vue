...
<template>
  <main class="bg-white px-4 pt-16 pb-20 sm:px-6 lg:px-8 lg:pt-24 lg:pb-28">
    <div class="mx-auto max-w-lg lg:max-w-7xl">
      <!-- <div class="border-b border-b-grey-200 pb-6">
        <h2
          class="text-3xl font-semibold tracking-tight text-gray-900 sm:text-4xl">
          Это Блог
        </h2>
      </div> -->
      <div class="mt-12 grid gap-16 lg:grid-cols-1 lg:gap-x-5 lg:gap-y-12">
        <div
          v-for="article in qc"
          :key="article._path"
        ...
        >
          <nuxt-link :href="article._path">
            <!-- <p class="text-xxl text-gray-900">{{ article.title }}</p> -->
            <p class="text-xl text-gray-900"><img :src="`${article.imgurl}`" :alt="`${article.title}`"></p>
